...
function SeqDets = MakeSeqMEG(ExpDets)
...
for i=1:ExpDets.numRun.meg
    SeqMatchType.Run{i}=randPermPick(2,ExpDets.numBlock.meg*ExpDets.numMatch);
end
...
for i=1:ExpDets.numExpBlock.meg
    ...
    SeqSampleISI = ExpDets.ISI.Sample.meg(randPermPick(length(ExpDets.ISI.Sample.meg),ExpDets.numSample));
    
    %-create sequence of index for match ISI in an experimental block
    SeqMatchISI = ExpDets.ISI.Match.meg(randPermPick(length(ExpDets.ISI.Match.meg),ExpDets.numMatch));
    ...
    while(matchDist)
        ...
        for j=1:(ExpDets.numMatch-1)
            if SeqMatchPlace(j+1)-SeqMatchPlace(j)<=ExpDets.matchDist.meg
                matchDist=true;
            end
        end
    end
